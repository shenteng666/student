...
app.controller('gradeController', function($scope, $http) {
	...
	$scope.findAll = function() {
		$http.get('../grade/findAll.do').success(function(response) {
			$scope.list = response;
		});
	}
    ...
    $scope.findPage=function(page,rows){//page:当前页码,rows:每页显示记录数
        $http.get('../grade/findPage.do?page='+page+'&rows='+rows).success(
            function(response){
	...
	$scope.save=function(){
			...
			$http.post('../grade/'+ methodName +'.do',$scope.entity ).success(
					function(response){
						if(response.success){
							   //重新查询 
					            $scope.reloadList();//重新加载
	...
	$scope.findOne=function(id){
	    $http.get('../grade/findOne.do?id='+id).success(
	        function(response){
	...
	//批量删除
	       $scope.dele=function(){
	           //获取选中的复选框
	           $http.get('../grade/delete.do?ids='+$scope.selectIds).success(
	               function(response){
	                   if(response.success){
	       ...
	       $scope.search=function(){
	    	   $http.post('../grade/search.do',$scope.searchEntity).success(
		               function(response){
		            	   $scope.list=response;
		               }
		           );
